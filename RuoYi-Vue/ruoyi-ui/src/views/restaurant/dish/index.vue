<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="dishList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="菜品ID" align="center" prop="dishId" width="70" />
      ...
      <el-table-column label="售价" align="center" prop="price" width="80">
        <template slot-scope="scope">
          ¥{{ scope.row.price }}
        </template>
      </el-table-column>
...
import { listCategory } from "@/api/restaurant/category"
...
export default {
  ...
  methods: {
    /** 加载分类列表 */
    getCategoryList() {
      listCategory({ pageNum: 1, pageSize: 999, status: '0' }).then(response => {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要' + text + '菜品"' + row.name + '"吗？').then(() => {
        return updateDish({ dishId: row.dishId, status: row.status })
      }).then(() => {
    ...
    /** 配方按钮 - 跳转到配方维护页 */
    handleRecipe(row) {
      this.$router.push({ path: '/restaurant/recipe', query: { dishId: row.dishId, dishName: row.name } })
    },
    /** 导出按钮操作 */
    handleExport() {
